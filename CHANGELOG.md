...
### Changed
- `disbug watch` now polls incrementally, prints a compact session URL/title/all-pin-feedback JSONL record, and no longer records an agent pickup while checking for new sessions.

### Added
- CLI: `login`, `logout`, `whoami`, `doctor`, `sessions`, `session`, `resolve`, `pin`, `pins`, `search`, `mcp`, `version`, `completion`
- MCP server with 7 read-only tools plus `resolve_session`, a narrowly scoped write tool that requires a fix summary
