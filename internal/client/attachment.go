...
// DownloadedAttachment contains attachment bytes and sanitized response metadata.
...
func (c *Client) DownloadAttachment(
	...
	if pinRef.Pin <= 0 {
		return nil, fmt.Errorf("pin number must be greater than zero")
	}
	...
	downloadPath := scopedSessionPath(pinRef.Session) + fmt.Sprintf(
		"pins/by-number/%d/attachments/%d/download/",
		pinRef.Pin,
		attachmentID,
	)
	absoluteURL := c.absoluteURL(downloadPath)
...
func attachmentFilename(resp *http.Response, attachmentID int64) string {
	if disposition := resp.Header.Get("Content-Disposition"); disposition != "" {
		if _, params, err := mime.ParseMediaType(disposition); err == nil {
			if filename := sanitizeAttachmentFilename(params["filename"]); filename != "" {
				return filename
			}
		}
	}
